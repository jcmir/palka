...
* `Executing` — дедлайн наступил, служба пересекла границу долговременной передачи на исполнение и зафиксировала это в персистентном снимке состояния; действие более не подлежит отмене; платформенный вызов еще может быть не вызван (сбой до вызова), может находиться в процессе выполнения (сбой во время вызова) либо завершился, но его терминальный результат еще не зафиксирован на диске (сбой до фиксации). Состояние `Executing` **НЕ означает** обязательный старт платформенного вызова, физический успех на платформе, завершение отключения питания или подтверждение сетевого состояния;
* `Completed` — платформенный шлюз подтвердил успешный прием/исполнение операции: для `BlockInternet` — физическое подтверждение состояния `Blocked` через `current_state`; для `ShutdownComputer` — успешный прием запроса на выключение шлюзом ОС (`initiate_shutdown() -> Ok(())`, что не означает факт завершения отключения питания). Является терминальным состоянием, действие завершено и удаляется из активного набора на диске;
* `Failed { reason: String }` — попытка исполнения запланированного действия завершилась ошибкой платформенного слоя (персистентное нетерминальное состояние, подлежащее повторным попыткам);
* `Missed` — терминальный исход восстановления для действия `ShutdownComputer`, чей дедлайн наступил во время недоступности системы/службы (выключенный ПК, перезагрузка, перезапуск службы), когда повторное исполнение отключения питания свежезагруженного компьютера является небезопасным или неактуальным (таймер аннулируется, удаляется из активных, отправляется уведомление `MissedDeadlineOccurred`).
...
* `InProgress` — системный вызов завершения работы принят ОС Windows в момент наступления дедлайна.
...
* `MessageSender` — перечисление участников диалога чата. Содержит **ИСКЛЮЧИТЕЛЬНО** варианты:
  * `Parent` — сообщение отправлено родителем;
  * `Child` — сообщение отправлено ребенком.
  Варианты `Service`, `System` или `Bot` в `MessageSender` отсутствуют.
...
Нормативная структура сообщения исключительно между родителем и ребенком:
...
* `ServiceReady` — инициализация успешно завершена (персистентное состояние загружено и валидировано, дескрипторы безопасности настроены, первичное согласование `DesiredInternetState` с платформой выполнено, служба готова к обработке команд).
